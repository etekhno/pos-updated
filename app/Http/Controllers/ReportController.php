...
class ReportController extends Controller
{
    ...
    public function profitLoss(Request $request)
    {
        ...
        // This discount column is actually consider as porfit / margin / revenue (speciall in sale)
        $purchase_dis = Purchase::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('total_discount');
        $profit_dis = Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('total_discount');
        $purchase_return_dis = ReturnPurchase::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('total_discount');
        $return_dis = Returns::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('total_discount');
    ...
    public function dueReportByCustomerId($id)
    {
        $customer_due_data = Sale::where('payment_status', '!=', 4)->where('customer_id', $id)->get();
        $dues = 0;
