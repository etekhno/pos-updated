...
class SaleController extends Controller
{
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            $lims_biller_list = Biller::where('is_active', true)->get();
            $lims_tax_list = Tax::where('is_active', true)->get();
            $lims_product_list = Product::select('id', 'name', 'code', 'image', 'product_details')->ActiveFeatured()->whereNull('is_variant')->get();
            foreach ($lims_product_list as $key => $product) {
                $images = explode(",", $product->image);
                $product->base_image = $images[0];
            }
            $lims_product_list_with_variant = Product::select('id', 'name', 'code', 'image', 'product_details')->ActiveFeatured()->whereNotNull('is_variant')->get();
        ...
        if(($category_id != 0) && ($brand_id != 0)){
            $lims_product_list = DB::table('products')
                                ...
                                ->where([
                                    ...
                                    ['products.is_active', true],
                                    ['brand_id', $brand_id]
                                ])->select('products.name', 'products.code', 'products.image', 'products.product_details')->get();
        }
        elseif(($category_id != 0) && ($brand_id == 0)){
            $lims_product_list = DB::table('products')
                                ...
                                ->where([
                                    ...
                                    ['categories.parent_id', $category_id],
                                    ['products.is_active', true]
                                ])->select('products.id', 'products.name', 'products.code', 'products.image', 'products.is_variant', 'products.product_details')->get();
        }
        elseif(($category_id == 0) && ($brand_id != 0)){
            $lims_product_list = Product::where([
                            ...
                            ->select('products.id', 'products.name', 'products.code', 'products.image', 'products.is_variant', 'products.product_details')
                            ->get();
        }
        else
        ...
        foreach ($lims_product_list as $product) {
            if($product->is_variant) {
                ...
                foreach ($lims_product_variant_data as $key => $variant) {
                    ...
                    $data['product_details'][$index] = $product->product_details;
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        if($lims_product_data->type == 'standard'){
            ...
            $unit_sale_on_percent_or_fix = array();
            foreach ($units as $unit) {
                if($lims_product_data->sale_unit_id == $unit->id) {
                    array_unshift($unit_name, $unit->unit_name);
                    array_unshift($unit_operator, $unit->operator);
                    array_unshift($unit_operation_value, $unit->operation_value);
                    array_unshift($unit_sale_on_percent_or_fix, $unit->percent_or_fix);
                }
                else {
                    $unit_name[]  = $unit->unit_name;
                    $unit_operator[] = $unit->operator;
                    $unit_operation_value[] = $unit->operation_value;
                    $unit_sale_on_percent_or_fix[] = $unit->percent_or_fix;
                }
            }
            $product[] = implode(",",$unit_name) . ',';
            $product[] = implode(",",$unit_operator) . ',';
            $product[] = implode(",",$unit_operation_value) . ',';     
            $product[] = implode(",",$unit_sale_on_percent_or_fix) . ',';
        }
        else{
            $product[] = 'n/a'. ',';
            $product[] = 'n/a'. ',';
    ...
    public function genInvoice($id)
    {
        ...
        $customer_due_data = Sale::where('payment_status', '!=', 4)->where('customer_id', $lims_sale_data->customer_id)->get();
        $dues = 0;

        foreach ($customer_due_data as $due) {
            $dues += $due->grand_total - $due->paid_amount;
        }
        ...
        return view('sale.invoice', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'dues'));
    }
