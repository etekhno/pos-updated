                    ...
                    <div class="card-body">
                        ...
                        <form id="product-form">
                            ...
                            <div class="row">
                                ...
                                <!-- product percentage will be used only for percentage objects -->
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label><?php echo e(trans('file.Unit Sale/Purchase on %')); ?> </strong> </label>
                                        <input type="checkbox" disabled name="percent_or_fix" checked="<?php echo e($lims_product_data->sale_percentage); ?>" required class="form-control">
                                        <span class="validation-msg"></span>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                     <div class="form-group">
                                        <label><?php echo e(trans('file.Sale %')); ?> </strong> </label>
                                        <input type="number" name="sale_percent" value="<?php echo e($lims_product_data->sale_percentage); ?>" required class="form-control" step="any">
                                        <span class="validation-msg"></span>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                     <div class="form-group">
                                        <label><?php echo e(trans('file.Purchase %')); ?> </strong> </label>
                                        <input type="number" name="purchase_percent" value="<?php echo e($lims_product_data->purchase_percentage); ?>" required class="form-control" step="any">
                                        <span class="validation-msg"></span>
                                    </div>
                                </div>
...
<script type="text/javascript">
    ...
    $('select[name="sale_unit_id"]').on('change', function() {
        
        unitID = $(this).val();
        if(unitID) {
            populate_unit_detail(unitID);
        }else{
            $('input[name="percent_or_fix"]').prop('checked', false);
            $('input[name="sale_percent"]').prop('disabled', true);
            $('input[name="purchase_percent"]').prop('disabled', true);
        }                        
    });
    ...
    function populate_unit_detail(unitID){
        $.ajax({
            url: '../saleunitdetail/'+unitID,
            type: "GET",
            dataType: "json",
            success:function(data) {
                console.log(data);
                if(data['percent_or_fix'] == true){
                    $('input[name="percent_or_fix"]').prop('checked', true);
                    $('input[name="sale_percent"]').prop('disabled', false);
                    $('input[name="purchase_percent"]').prop('disabled', false);
                }else{
                    $('input[name="percent_or_fix"]').prop('checked', false);
                    $('input[name="sale_percent"]').prop('disabled', true);
                    $('input[name="purchase_percent"]').prop('disabled', true);
