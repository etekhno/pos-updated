...
<!-- information for the profit / loss -->
			<div class="col-md-4">
				<div class="colored-box">
					<i class="fa fa-dollar"></i>
					<h3>{{trans('file.Estimate Profit/loss')}}</h3>
					<hr>
					<div class="mt-3">
						<p class="mt-2">{{trans('file.Sale Revenue')}} <span class="float-right"> {{number_format((float)($profit_dis), 2, '.', '') }}</span></p>
						<p class="mt-2">{{trans('file.Sale Return Revenue')}} <span class="float-right">- {{number_format((float)$return_dis, 2, '.', '')}}</span></p>
						<p class="mt-2">{{trans('file.Expense')}} <span class="float-right">- {{number_format((float)$expense, 2, '.', '')}}</span></p>
						<p class="mt-2">{{trans('file.Payroll')}} <span class="float-right">- {{number_format((float)$payroll, 2, '.', '')}}</span></p>
						<p class="mt-2">{{trans('file.Estimate Profit/loss')}} <span class="float-right">{{number_format((float)($profit_dis - $return_dis - $expense - $payroll), 2, '.', '') }}</span></p>
					</div>
				</div>
			</div>
